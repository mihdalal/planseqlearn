...
for idx, env_name in enumerate(env_names):
    ...
    for _ in range(num_seeds):
        ...
        run_name = f"drqv2_psl_replicate_v3_{env_name}"
        cmd = f"python planseqlearn/train.py agent=drqv2 use_wandb=True seed={seed} experiment_subdir=psl_replicate_v3 debug=True save_video=True num_train_frames={num_train_frames} camera_name='robot0_eye_in_hand' eval_every_frames=10000 num_eval_episodes=10 wandb.project_name=psl_replicate_v3 wandb.run_name={run_name} task={env_name} valid_obj_names={valid_obj_names} psl=True path_length={path_length} action_repeat=1 experiment_id={run_name} matrix=True"
        os.system(cmd)
        print(cmd)
        time.sleep(5)
