...
class V2Agent:
    def __init__(
        ...
        self.robot_mask_decoder = PoolDecoder(
            in_channels=16,
    ...
    def update_critic_and_decoders(
        self, encoded_obs, action, reward, discount, encoded_next_obs, step, robot_masks
    ):
        metrics = {}
        ...
        robot_mask_loss = self.robot_mask_loss_fn(reconstructed_robot_mask, robot_masks)
        robot_mask_loss = (
        ...
        loss = (
            critic_loss
            + non_robot_mask_loss * self.mask_loss_coeff
            + robot_mask_loss * self.mask_loss_coeff
        )
    ...
    def update(self, replay_iter, step):
        ...
        metrics.update(
            self.update_critic_and_decoders(
                encoded_obs, action, reward, discount, encoded_next_obs, step, masks
            )
        )
    ...
    def get_frames_to_record(self, obs):
        ...
        with torch.no_grad():
            ...
            f1 = encoded_obs[:, : int(encoded_obs.shape[1] / 2)]
            f2 = encoded_obs[:, int(encoded_obs.shape[1] / 2) :]
            reconstructed_non_robot_mask = (
                self.non_robot_mask_decoder(f1).detach().cpu().numpy()
            )
            reconstructed_mask = self.robot_mask_decoder(f2).detach().cpu().numpy()
