...
from planseqlearn.models.decoder import SacAeDecoder
from planseqlearn.models.encoder import DrQV2Encoder
...
class V4Agent:
    def __init__(
        ...
        self.mask_decoder = SacAeDecoder(
            in_channels=16,
            out_channels=obs_shape[0] // 3,
            output_act=nn.Sigmoid(),
            repr_dim=int(feature_dim / 2),
        ).to(device)
        self.decoder = SacAeDecoder(
            in_channels=16, out_channels=obs_shape[0], repr_dim=int(feature_dim / 2)
        ).to(device)
        self.actor = Actor(
