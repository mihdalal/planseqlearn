...
class V1RandomMaskAgent:
    ...
    def generate_random_mask(self):
        patch_rows = np.random.randint(
            0,
            self.frame_resolution - (self.patch_size - 1),
            self.num_patches,
            dtype=int,
        )
        patch_cols = np.random.randint(
            0,
            self.frame_resolution - (self.patch_size - 1),
            self.num_patches,
            dtype=int,
    ...
    def update_critic_and_decoders(
        ...
        reconstructed_obs = self.decoder(f1.detach() if self.detach_decoders else f1)
        ...
        mask_loss = self.mask_loss_fn(
            reconstructed_mask, self.fixed_target_mask.expand(reconstructed_mask.shape)
        )
        mask_loss = mask_loss.reshape(reconstructed_mask.shape[0], -1).sum(dim=1).mean()
    ...
    def load_pretrained_weights(self, pretrain_path, just_encoder_decoders):
        ...
        self.mask_decoder.load_state_dict(pretrained_agent.mask_decoder.state_dict())
